...
function AttendanceTab({ tenantId }: { tenantId: string }) {
  ...
  const printSalarySlip = async (s: CarwashStaff) => {
    ...
    <div class="header">${logo ? `<div><img src="${logo}" class="logo" /></div>` : ''}<div class="shop">${shopName}</div>${config?.address_line1 ? `<div style="font-size:11px;color:#6b7280">${config.address_line1}${config.city ? ', ' + config.city : ''}</div>` : ''}</div>
