...
export function POSPage() {
  ...
  const invoiceTemplate = {
    ...
    addressLine: settings.invoiceAddressLine || [config?.address_line1, config?.city].filter(Boolean).join(', ') || '',
    headerLeft: settings.invoiceHeaderLeft || 'Chemist & Druggist',
    headerRight: settings.invoiceHeaderRight || 'Cash/Credit Memo',
    whatsappNumber: settings.invoiceWhatsappNumber || '',
    footerNote: settings.invoiceFooterNote || 'Thanks for your purchase',
    signatureLabel: settings.invoiceSignatureLabel || 'Authorised Signature',
  };
  const keyboardBillingMode = isMedicalStore || settings.enableKeyboardBillingMode === true;
  const canEditInvoiceDateTime = isMedicalStore;
  ...
  const { data: customerSearchResultsRaw = [], isFetching: isFetchingCustomers } = useQuery({
    queryKey: ['pos-customer-search', trimmedCustomerSearch, tenantId],
    queryFn: () => listCustomers(tenantId, { search: trimmedCustomerSearch, perPage: 8 }).then((r) => r.items),
    enabled: trimmedCustomerSearch.length > 1 && !!tenantId,
  });
  const customerSearchResults = customerSearchResultsRaw;

  const trimmedCreditCustomerSearch = creditCustomerSearch.trim();
  const { data: creditCustomerResultsRaw = [] } = useQuery({
    queryKey: ['pos-credit-customer-search', trimmedCreditCustomerSearch, tenantId],
    queryFn: () => listCustomers(tenantId, { search: trimmedCreditCustomerSearch, perPage: 8 }).then((r) => r.items),
    enabled: trimmedCreditCustomerSearch.length > 1 && !!tenantId,
  });
  const creditCustomerResults = creditCustomerResultsRaw;
  ...
  const handleSearchKeyDown = async (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && trimmedSearch) {
      ...
      try {
        const product = await getProductByBarcode(tenantId, trimmedSearch);
  ...
  const placeMutation = useMutation({
    mutationFn: async (_paymentRef?: string) => {
      ...
      if (!hasBillableItems) throw new Error('Add at least one product to continue');
      ...
      const billableItems = cart.items.filter((i) => getCartItemBillableQuantity(i) > 0);
      ...
      return createOrder(tenantId, {
        ...
        items: billableItems.map((i) => {
          ...
          const avgPrice = combinedQty > 0 ? combinedSubtotal / combinedQty : i.unitPrice;
          const itemTotal = avgPrice * combinedQty - i.discount;
          return {
        ...
        subtotal,
        discount: cart.items.reduce((s, i) => s + (i.discount ?? 0), 0),
        tax_total: taxTotal,
        total,
  ...
  const createProductMutation = useMutation({
    ...
    onSuccess: (product) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['pos-top-products'] });
      navigate('/inventory', {
        state: { openAdjustStock: true, productId: product.id, productName: product.name, direction: 'add', type: 'purchase', returnTo: '/pos' },
      });
    },
  ...
  const createCreditCustomerMutation = useMutation({
    mutationFn: async ({ name, phone }: { name: string; phone: string }) => {
      ...
      return createCustomer(tenantId, { name: name.trim(), phone: phone.trim() || null, email: null, address: null, city: null, tags: [], credit_limit: 0, notes: null });
    },
    onSuccess: (customer) => {
      cart.setCustomer(customer.id, customer.name ?? customer.phone ?? 'Customer', customer.phone ?? null);
      setCreditCustomerSearch('');
    ...
    onError: (err: any) => toast.error(err.message ?? 'Unable to create customer'),
  });
  ...
  const buildInvoiceSummary = () => {
    ...
    return [
      `${invoiceTemplate.storeDisplayName || config?.shop_name || 'Medical Invoice'}`,
...
<script>window.onload = () => { window.print(); }<\/script>
  ...
  const resultLoading = isFetching;
